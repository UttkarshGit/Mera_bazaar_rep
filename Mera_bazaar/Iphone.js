...
import iphone from './json/mobile_iphone_11.json';
      ...
      <h1 style={{'marginTop':'1%','textAlign':'center','backgroundColor':'rgb(236, 250, 250)'}}>{iphone[0].product_name}</h1>
        {
          iphone.map((e) => {
            return <div className="card mb-3" style={{ "maxWidth": "540px",'border':'1px solid black'}}>
             ...
             <a className="btn btn-outline-success mx-3" onClick={()=>{localStorage.setItem("product_name",e.product_name);localStorage.setItem('product_image',e.image);localStorage.setItem('product_prize',e.prize)}} style={{'textDecoration':"none",'color':'black','marginBottom':'3%','marginTop':'3%'}} href="/payment">Want to buy</a>
            </div>
          })
}
</div>
