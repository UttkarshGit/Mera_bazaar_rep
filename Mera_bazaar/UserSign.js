...
function UserSign()
{
    ...
    {
        let signup_data= await fetch("http://localhost:4000/seller_register",{
            method:'post',
            body:JSON.stringify({name,email,password,aadhaar,pan}),
            headers:{
    ...
    return(
        <>
        <h1 style={{'textAlign':'center','marginTop':'5px','color':'white'}}>Seller Sign Up</h1>
  ...
  <div className="mb-3 form-group w-60 topp">
    <label htmlFor="exampleInputEmail1" className="form-label"><b>Aadhaar No.</b></label>
    <input type="text" onChange={(e)=>{setaadhaar(e.target.value)}} className="form-control" id="exampleInputEmail1" aria-describedby="emailHelp"/>
  </div>
  <div className="mb-3 form-group w-60 topp">
    <label htmlFor="exampleInputEmail1" className="form-label"><b>Pan Card No.</b></label>
    <input type="text" onChange={(e)=>{setpan(e.target.value)}} className="form-control" id="exampleInputEmail1" aria-describedby="emailHelp"/>
  </div>
